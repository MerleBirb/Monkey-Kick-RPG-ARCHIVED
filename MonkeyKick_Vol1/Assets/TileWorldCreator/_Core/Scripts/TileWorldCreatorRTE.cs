...
namespace TileWorld
{
    public class TileWorldCreatorRTE : MonoBehaviour
    {
        ...
        {
            string _finalPath = System.IO.Path.Combine(Application.dataPath, System.IO.Path.Combine(path, _name + fileExtension));
        ...
        void ShowGUILoad()
        {
            ...
            string _finalPath = System.IO.Path.Combine(Application.dataPath, path);
            ...
            for (int i = 0; i < _fileInfo.Length; i++)
            {
                ...
                if (GUI.Button(new Rect(0, 0 + (30 * i), 180, 25), System.IO.Path.GetFileName(_fileInfo[i])))
                {
                    Load(_fileInfo[i]);
